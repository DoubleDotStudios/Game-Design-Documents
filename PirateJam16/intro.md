# Introduction

## Game Summary Pitch
Shadowkeep is a topdown, autoscrolling boss rush where you are the dungeon. You control the booby traps to eliminate the monsters that block the path of a brave adventurer who wishes to take win treasure.

## Inspiration

#### [Weapon Exchange Box](https://zye-3.itch.io/weapon-exchange-box)
A fun puzzle boss rush game. Each boss is unique and has phases only suceptible to a color. You must pick and choose your weapons wisely as you wneture through each room to defeat bosses. This is the inspiration for the boss rooms that you control.

#### [TABS](https://store.steampowered.com/app/508440/Totally_Accurate_Battle_Simulator/)
A battle simulator where you control troops with uniqueattacks to defend and defeat other armies. Strategy is involved due to a limited amount of money that can be spent on troops. The inspiration from this is the control over what you use with the limitation of a cooldown.

## Player Experience
For 4 rooms you must strategically use your traps to defeat the bosses. Each of your traps has a cooldown so be strategic about what you use at what time. Each boss has a unique set of attacks so be ready for anything. I your adventurer is to perish then you'll have to start again and learn from your mistakes.

## Platform
The game is developed primarily for Web.

## Development Software
- Godot 4.3:   Programming
- Libresprite: Assets + GUI
- Bosca Ceoil: Music
- Jsfxr:       Sfx

## Genre
- Boss rush
- Strategy

## Target Audience
With simple elements of planning but fast paced gameplay, the game is geared for those who enjoy the Boss Rush genre but want a new challenge. And, for strategy players who like a more offensive side to their ideas.
